<template>
  <aside ref="sidebarRef" class="sidebar">
    ...
    <div v-if="showTagFilter && tagDisplayData.length" class="tag-filter-area">
      <span v-for="item in tagDisplayData" :key="item.filter" class="tag-pill-wrapper">
        <button
          class="tag-pill"
    ...
    >
      <p class="summary-text" v-html="marked.parseInline(config.summary.content)" />
    </div>

    <!-- Configurable Sections -->
    <template v-for="(section, index) in config.sections" :key="section.label">
      <div v-if="section.entries.length" class="section-label" :class="{ 'section-label--education': index > 0 }">
        {{ section.label }}
      </div>
      <div
      ...
      >
        <div class="entry-top-line">
          ...
          <span v-if="section.showDates !== false" class="entry-dates">{{ formatDateRange(entry.StartDate, entry.EndDate) }}</span>
        </div>
        <div class="entry-title">{{ entry.Title }}</div>
        <div class="entry-summary" v-html="marked.parseInline(entry.Summary)" />
      </div>
    </template>
...
import { buildTagDisplay } from '@/composables/useTagAggregation.js'
...
const failedQuery    = ref(null)

// All entries pool for tag lookups
const allEntries = computed(() =>
  props.config.sections.flatMap((s) => s.entries || [])
)
